# -*- coding: utf-8 -*-
"""
Created on Fri Jul 29 20:50:10 2022

@author: Name: Andres David Valdes Gonzalez
ID: A00062146
"""


class Car():
    def __init__(self, colour='NA', brand='NA', topSpeed=0, currentSpeed=0, up=0, down=0):
        self.colour= colour
        self.brand= brand
        self.topSpeed= topSpeed
        self.currentSpeed = currentSpeed
        self.up = up
        self.down = down
     
#Methods
    def start(self):
        self.currentSpeed + self.currentSpeed
        return self.currentSpeed
    def accelerate(self):
        self.currentSpeed +=self.up
        return self.currentSpeed
    def brake(self):
        self.currentSpeed -=self.down
        return self.currentSpeed
    def stop(self):
        self.currentSpeed -=self.currentSpeed
        return self.currentSpeed
    def get_speedup(self,speedup):
        self.up = speedup
    def set_speed(self):
        return self.set_speed()
    def get_speeddown(self,speeddown):
        self.down = speeddown
    def set_speeddown(self):
        return self.set_speed()    

#Display the speed

    def display(self):
        print('Car color is:', my_car.colour,'\nBrand: ', my_car.brand,\
              "\nMax speed:", my_car.topSpeed, '\nCurrent speed:', my_car.currentSpeed,'\n')


my_car = Car('red','Hundai',100,0)#Initial speed is = 0

my_car.start()
my_car.get_speedup(50)#Increase speed given is 50
my_car.accelerate()
my_car.display()

my_car.get_speeddown(45)#Decrease speed given is 5
my_car.brake()
my_car.display()